...
uses
  ...
  ppDBBDE, ppTypes, SOLPreview, inifiles, ppStrtch, ppSubRpt, Printers, ppForms,
  variants, StrContainer, fcLabel, RecError;
...
//
  ...
  public
    ...
    function  Execute ( sTabla, sModulo, sAdmin,
                        sUser, sFilter, sFile, sSection  : String;
                        wcdsj : TwwClientDataSet;
                        wDCOM : TDCOMConnection
                      ): Boolean;
...
var
  ErrorCount : Integer;
  DCOMT     : TDCOMConnection;
  SolMantenimiento: TSolMantenimiento;

implementation

uses SOLRef02, SOLRef03;
...
function TSolMantenimiento.Execute ( sTabla, sModulo, sAdmin,
                                     sUser, sFilter, sFile, sSection  : String;
                                     wcdsj : TwwClientDataSet;
                                     wDCOM : TDCOMConnection
                                   ): Boolean;
VAR
   ...
   tSQL : String; wClassSrv : PChar; wResult : string;
begin
...
{     wResult:=wDCOM.AppServer.SOLEnlace1(sModulo);
     ...
     if sModulo='GRH'  then wClassSrv:='SOL GRHsrv';
     if sModulo='PPRE' then wClassSrv:='SOL PPREsrv';
  ...
  wGUID:=1;
  if sModulo='LOG'  then if wDCOM.ServerGUID='{EAF516E3-0B10-11D4-A742-0004ACD308FC}' then else wGUID:=0;
  if sModulo='ALM'  then if wDCOM.ServerGUID='{294DA083-D58B-11D3-B576-10005ABAB7A3}' then else wGUID:=0;
  if sModulo='CXP'  then if wDCOM.ServerGUID='{294DA083-D58B-11D3-B576-10005ABAB7A3}' then else wGUID:=0;
  if sModulo='FAC'  then if wDCOM.ServerGUID='{294DA083-D58B-11D3-B576-10005ABAB7A3}' then else wGUID:=0;
  if sModulo='CXC'  then if wDCOM.ServerGUID='{294DA083-D58B-11D3-B576-10005ABAB7A3}' then else wGUID:=0;
  if sModulo='CAJA' then if wDCOM.ServerGUID='{294DA083-D58B-11D3-B576-10005ABAB7A3}' then else wGUID:=0;
  if sModulo='CNT'  then if wDCOM.ServerGUID='{294DA083-D58B-11D3-B576-10005ABAB7A3}' then else wGUID:=0;
  ...
  if sModulo='PPRE' then if wDCOM.ServerGUID='{294DA083-D58B-11D3-B576-10005ABAB7A3}' then else wGUID:=0;
  if sModulo='SEG'  then if wDCOM.ServerGUID='{294DA083-D58B-11D3-B576-10005ABAB7A3}' then else wGUID:=0;

  if sModulo='APO'  then if wDCOM.ServerGUID='{6893A1E6-2B7F-473E-8C71-1DE0495B9B23}' then else wGUID:=0;
  if sModulo='CRE'  then if wDCOM.ServerGUID='{EF8501A3-94AE-11D6-9A80-005004FC893C}' then else wGUID:=0;
  if sModulo='COB'  then if wDCOM.ServerGUID='{7A6C7183-94BF-11D6-9A80-005004FC893C}' then else wGUID:=0;
  if sModulo='PRE'  then if wDCOM.ServerGUID='{7A6C7188-94BF-11D6-9A80-005004FC893C}' then else wGUID:=0;
...
var
  wQuery : string;
begin
//  If wAdmin='S' then begin
  If wAdmin<>'U' then begin
  ...
  begin
     ...
     cds1.FieldByName('CODIGO'      ).Visible := False;
     cds1.FieldByName('ZARCHIVO'    ).Visible := False;
     cds1.FieldByName('ZPROCEDURE'  ).Visible := False;
     cds1.FieldByName('LISTAREPORT' ).Visible := False;
     cds1.FieldByName('LISTADETALLE').Visible := False;
     cds1.FieldByName('ACCESOS'     ).Visible := False;
...
begin
   ...
   if (cds1.fieldByName('EDITA').AsString='N') or (cds1.fieldByName('EDITA').AsString='') then Z2bbtnModifica.Enabled:=False;

   if Z2bbtnModifica.Enabled then begin
      pnlRegistro.Enabled:=True;
      bbtnOk.Enabled:=True;
...
procedure TSolMantenimiento.FormActivate(Sender: TObject);
begin
  if wTabla='' then
     dbgTables.Enabled:=True;
...
begin
   ...
   If pnlFiltro.Visible then begin
      ...
      imgTacho.Left    := pnlFiltro.Width-imgTacho.Width-bbtnExporta.Width-70;
      edtRegistros.Left:= pnlFiltro.Width-edtRegistros.Width-bbtnExporta.Width-70;
      lblRegistros.Left:= pnlFiltro.Width-lblRegistros.Width-bbtnExporta.Width-70;
      ...
      GeneraListBox;
//      edtRegistros.Text:=formatfloat('###,###,###',cds2.RecordCount);
      end
   else begin
      dbgFiltro.Height:= dbgFiltro.Height + pnlFiltro.Height-1;
...
begin
   ...
   if Length( wUserSQL )>0 then begin
      ...
      if Length( xOrder ) > 0 then begin
         if Pos('ORDER BY', UpperCase( wQuery ))<=0 then begin
            wQuery  := wQuery + ' ORDER BY ' + xOrder;
         end
         else begin
...
var
   wFieldName, wFieldTitle, wFieldPict, xxxCampo  : String;
      ...
      begin
         ...
         if Trim(wFieldPict)<>'' then begin
            ...
            if cdsGrid.FieldByName('TIPOCOMBO').AsString='D' then begin
               ...
               //(cds2.FieldByName(wFieldName) AS TDateTimeField).EditFormat   := wFieldPict;
            end
            else begin
...
procedure TSolMantenimiento.lbFiltroEndDrag(Sender, Target: TObject; X, Y: Integer);
begin
   if Target=imgTacho then begin
...
procedure TSolMantenimiento.lbFiltroMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TSolMantenimiento.lbOrdenEndDrag(Sender, Target: TObject; X, Y: Integer);
begin
   if Target=imgTacho then begin
...
procedure TSolMantenimiento.lbOrdenMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
var
   ...
   xCAM : String;
begin
   if cdsReporte.FieldByName( 'CAMPODES' ).AsString<>'' then begin
      ...
      while AnsiPos( ';', xVAR )>0 do begin
         ...
         For xcr:=0 to clbRepD.Items.Count-1 do begin
             if clbRepDReal.Items[ xcr ]=xCAM then begin
...
begin
   ...
   if wUserSQL<>'' then begin
      ...
      if xLenS>0 then begin
         xSQLTmp:=Copy(wUserSQL, 1, xLenS-1 )
      end;
...
//   cdsUsuarioAct.Filter  :='USERID='''+DM1.wUsuario+''' AND TABLANOMBRE='''+xxArchivo+'''';
...
//      ( not FileExists( wRutaCds+DM1.wModulo+xxCds.Name ) ) then
   xxCds.Close;
